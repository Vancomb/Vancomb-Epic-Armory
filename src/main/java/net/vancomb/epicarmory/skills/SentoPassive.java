...
public class SentoPassive extends PassiveSkill {
    ...
    public void onReset(SkillContainer container) {
        ...
        if (!executer.isLogicalClient() && (Boolean)container.getDataManager().getDataValue(SHEATH)) {
            ServerPlayerPatch playerpatch = (ServerPlayerPatch)executer;
            container.getDataManager().setDataSync(SHEATH, false, (ServerPlayer)playerpatch.getOriginal());
        ...
        if (!executer.isLogicalClient() && this.consumption < value) {
            ...
            if (!(Boolean)container.getDataManager().getDataValue(SHEATH)) {
                ...
                /*if (container.getExecuter().getSkill(WOMSkills.MEDITATION) == null) {
                    flag = true;
                } else if ((Integer)container.getExecuter().getSkill(WOMSkills.MEDITATION).getDataManager().getDataValue(MeditationSkill.TIMER) == 0) {
                    flag = true;
                }*/
    ...
    public float getCooldownRegenPerSecond(PlayerPatch<?> player) {
        return ((Player)player.getOriginal()).isUsingItem() ? 0.0F : 1.0F;
    }
}
